...
import { hexToBytes } from '@/ble/types';
...
export const MessageTypes = {
  ...
  TELEMETRY_STREAM: hexToBytes('553a0470'),
  /** Rep completion summary */
  REP_SUMMARY: hexToBytes('554a04c6'),
  /** Set completion summary */
  SET_SUMMARY: hexToBytes('554f0439'),
  /** Status update */
  STATUS_UPDATE: hexToBytes('553404ac'),
} as const;
...
export const TelemetryOffsets = {
  SEQUENCE: 6,    // 2 bytes, little-endian
  PHASE: 13,      // 1 byte
  POSITION: 24,   // 2 bytes, little-endian unsigned
  FORCE: 26,      // 2 bytes, little-endian signed
  VELOCITY: 28,   // 2 bytes, little-endian unsigned
} as const;
...
export enum MovementPhase {
  IDLE = 0,
  CONCENTRIC = 1,  // Pulling (muscle shortening)
  HOLD = 2,        // Top of rep / transition
  ECCENTRIC = 3,   // Releasing (muscle lengthening)
  UNKNOWN = -1,
}
