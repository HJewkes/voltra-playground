...
export function buildLoadVelocityProfile(
  ...
  let estimated1RM = slope !== 0 
    ? Math.round((mvt - intercept) / slope / 5) * 5
    : 0;
 ...
 */
export function estimateWeightForPercent1RM(
  profile: LoadVelocityProfile,
  percent: number
): number {
 ...
 */
export function predictVelocityAtWeight(
  profile: LoadVelocityProfile,
  weight: number
): number {
...
export function generateWarmupSets(
  ...
  const warmupSets: WarmupSet[] = [
    {
      weight: Math.round(estimated1RM * 0.40 / 5) * 5,
      reps: 10,
      purpose: 'Get moving, feel the groove',
    },
    {
      weight: Math.round(estimated1RM * 0.60 / 5) * 5,
      reps: 6,
...
export function estimate1RMFromSet(
  ...
  if (velocity && velocity > 0) {
    ...
    const percent = estimatePercent1RMFromVelocityLocal(velocity);
...
/**
 * Local helper to estimate %1RM from velocity (avoids circular import).
 */
function estimatePercent1RMFromVelocityLocal(velocity: number): number {
  // Simplified inverse lookup
  if (velocity >= 1.20) return 45;
  if (velocity >= 1.00) return 55;
  if (velocity >= 0.90) return 60;
  if (velocity >= 0.72) return 70;
  if (velocity >= 0.55) return 80;
  if (velocity >= 0.37) return 90;
  if (velocity >= 0.25) return 95;
  return 100;
}
