...
const DEFAULT_CONFIG: NativeAdapterConfig = {
  autoReconnect: true,
...
export class NativeBLEAdapter implements BLEAdapter {
  ...
  private async attemptReconnect(): Promise<void> {
    ...
    while (this.reconnectAttempts < this.config.maxReconnectAttempts) {
      ...
      console.log(`[NativeBLE] Reconnect attempt ${this.reconnectAttempts}/${this.config.maxReconnectAttempts}`);
  ...
  async scan(timeout: number): Promise<Device[]> {
    ...
    console.log('[NativeBLE] Looking for service:', BLE.SERVICE_UUID);
    console.log('[NativeBLE] Device name prefix:', BLE.DEVICE_NAME_PREFIX);
    ...
    return new Promise((resolve, reject) => {
      // First try scanning without service filter to see what's out there
      this.manager.startDeviceScan(
        ...
        (error, device) => {
          ...
          if (device) {
            ...
            // Check if it's a Voltra device
            const isVoltra = device.name?.startsWith(BLE.DEVICE_NAME_PREFIX) ||
                            device.localName?.startsWith(BLE.DEVICE_NAME_PREFIX);
            
            if (isVoltra && !seen.has(device.id)) {
              console.log(`[NativeBLE] ✓ Found Voltra: ${device.name || device.localName}`);
              seen.add(device.id);
              devices.push({
                id: device.id,
                name: device.name || device.localName || 'Voltra',
                rssi: device.rssi,
              });
            }
          }
      ...
      setTimeout(() => {
        ...
        console.log(`[NativeBLE] Scan complete. Found ${devices.length} Voltra device(s)`);
        resolve(devices);
      }, timeout * 1000);
    });
  }
